...
public class LinkedTree<T extends Comparable<T>> implements TreeADT<T> {
    ...
    public void addChild(T parent, T child) throws ElementNotFoundException {
        //create parentNode reference
        MultiTreeNode<T> parentNode = findNode(parent); 
        
        if (parent == null || child == null) { //Check if element has data
        } else if (parentNode == null) {
            throw new ElementNotFoundException //throw exception if no data
            ("ERROR: Element" + parentNode + "DNE!");
            
        } else {
            addChild(parentNode, child); //if everyhting has data,implement data
        }  
    }
    ...
    protected void addChild(MultiTreeNode<T> parentNode, T child) {
        //Create childNode refernce 
        MultiTreeNode<T> childNode = new MultiTreeNode(child);
        
        parentNode.addChild(childNode); //add childNode
    ...
    private int countDown(MultiTreeNode<T> node) {
        int cd = 1; //The count down int to return
        
        if (node == null) { //Check if there is data
            return 0;     
        } else if (node.getNumChildren() == 0) { //Check number of children 
            return 1;
        } else { //reduce cd if multiple children 
            cd = node.getChildren().stream().map((child) -> 
                    countDown(child)).reduce(cd, Integer::sum);
        }
