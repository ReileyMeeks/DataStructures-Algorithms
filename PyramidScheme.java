...
public class PyramidScheme extends LinkedTree<Person> {
    ...
    private ArrayList<MultiTreeNode<Person>>
            findNodeChain(MultiTreeNode<Person> node, 
                    Person target) {
         //creates target person reference
        ArrayList<MultiTreeNode<Person>> tPerson = new ArrayList<>();

        if (node.getElement().equals(target)) {
            return tPerson;
        } else {
            ArrayList<MultiTreeNode<Person>> temp; //create temp array 

            for (MultiTreeNode<Person> p : node.getChildren()) {
                temp = findNodeChain(p, target);
